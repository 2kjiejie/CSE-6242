...
val dff = df.filter($"PULocationID" =!= $"DOLocationID" && ($"trip_distance") > 2.0)
...
// ENTER THE CODE BELOW
var do_df = dff.groupBy("DOLocationID")
  .count()
  .orderBy(desc("count"))
  .show(5)
...
// ENTER THE CODE BELOW
var pu_df = dff.groupBy("PULocationID")
  .count()
  .orderBy(desc("count"))
  .show(5)
...
// ENTER THE CODE BELOW
var pu_df = dff.groupBy("PULocationID","DOLocationID")
  .count()
  .orderBy(desc("count"))
  .show(5)
...
val unionDF = pu_df.union(do_df)

val sortdf = unionDF.groupBy("PULocationID").sum("count").orderBy(desc("sum(count)"))
